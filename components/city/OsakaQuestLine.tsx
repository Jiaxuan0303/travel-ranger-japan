...
export function OsakaQuestLine() {
  ...
  const questIdMap: Record<string, string> = {
    'osaka-okonomiyaki': 'quest-osaka-1',
    'osaka-dotonbori': 'quest-osaka-2',
    'osaka-arcade': 'quest-osaka-3',
    'osaka-kuromon': 'quest-osaka-4',
  };
    ...
    <section className="mb-10">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-xs text-orange-400/60 ml-auto">
          {completedIds.length} / {osakaQuests.length}
        </span>
      </div>
              ...
              <div className={`relative overflow-hidden rounded-xl border p-4 transition-all duration-500 ${
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-orange-400/60">{quest.subtitle}</p>
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] text-white bg-gradient-to-r ${quest.skillColor}`}>
                        {quest.skillTag}
                      </span>
                      <span className="text-[10px] text-orange-400/60">⭐ {quest.xpReward}</span>
                    </div>
                  </div>
                </div>
